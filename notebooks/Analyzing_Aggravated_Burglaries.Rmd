...
```
```{r}
# Renaming tracts column in DC_tracts and convert to numeric datatype to simplify future merging and plots
DC_tracts <- DC_tracts |>
...
burglary_totals <- census_burglaries |> 
  ...
  select(tract, Num_Unique_Burglaries, population, Burglaries_Per_Pop, median_income) |>
...
To further investigate whether a Poisson model would be appropriate, we can look at the distribution of burglary rate in more detail. 
...
paste0('For 2 Davidson county census tracts with the same population, a increase in median income of $1,000 is associated with a ',pct_change,'% lower estimated average number of burglaries.')
```

Evaluating how well the model fits the data:

```{r}
exp(confint(burglaries_income_pop))
```
A 95% confidence interval for $e^{\beta_1}$ does not include 1, indicating median income is significantly associated with burglary rate.

```{r}
#Comparing model predictions to observed rates

# Get the burglary rate values that the model predicts
fitted_values <- predict(burglaries_income_pop, type = "response")
residuals <- residuals(burglaries_income_pop, type = "response")
...
deviance <- summary(burglaries_income_pop)$deviance
degrees_freedom <- summary(burglaries_income_pop)$df.residual
